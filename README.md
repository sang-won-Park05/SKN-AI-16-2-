...
| 안주영   | 프로젝트를 진행하기 전에 목표는 2가지 였다. 하나는 데이터에 맞는 좋은 성능의 모델을 찾는 것, 그리고 그 모델을 통해 산출물을 만드는 것이었다. 우리가 다루는 데이터에 적합한 모델을 찾긴 했으나 성능차이가 거의 없어서 모델별로 분석할 수 없어서 아쉬웠다. 하지만 학습된 모델을 통해 당원 이탈율을 예측까지 했다는 점에서는 프로젝트가 꽤 완성도 있다고 자체적으로 평가했다. 만약에 시간이 좀 더 충분했다면 관리 개인화 시스템까지 구축해서 고도화된 산출물을 제공할 수 있었을 거 같다.  |
